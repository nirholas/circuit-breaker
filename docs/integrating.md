# Integrating CircuitBreaker

## From Solidity

```solidity
import {CircuitBreakerHook} from "circuit-breaker/src/hooks/CircuitBreakerHook.sol";
...
hook.configure(key, CircuitBreakerHook.Config({ /* see the README */ }));
...
const address = hookAddress("circuit-breaker", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "CircuitBreaker"
...
More at https://circuit-breaker-1bp.pages.dev.
