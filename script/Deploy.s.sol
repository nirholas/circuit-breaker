 ...
 * @notice Deploys CircuitBreaker deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
...
contract DeployCircuitBreaker is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(CircuitBreakerHook).creationCode;
        ...
        console2.log("CircuitBreaker ", predicted);
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("CircuitBreaker")),
