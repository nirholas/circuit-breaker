...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(CircuitBreakerHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new CircuitBreakerHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, CircuitBreakerHook.Config({maxTickMove: 200, windowSeconds: 300, cooldownSeconds: 900}));
